...
app = typer.Typer(
    name="sharp",
    help="SHARP - System for Harnessing Augmented Reasoning and Transforming Heuristics",
...
def health() -> None:
    ...
    table.add_row("Circuit Breaker", f"OK - {cb.state}")

    bm = BudgetManager(HarnessConfig.default().safety)
    usage = bm.get_usage()
    table.add_row("Budget", f"OK - ${usage['session_cost']:.4f} / ${usage['cost_limit']:.2f}")

    try:
        import litellm
        table.add_row("LiteLLM", "OK - installed")
    except ImportError:
...
@app.command()
def benchmark(
    test: str = typer.Option("all", "--test", "-t", help="Benchmark test to run"),
) -> None:
    """Run benchmarks."""
    from sharp.harness.benchmarks.runner import run_benchmark
    asyncio.run(run_benchmark(test))
